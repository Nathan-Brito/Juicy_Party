...
screen say(who, what):
    ...
    if not renpy.variant("small"):
        add SideImage() xalign 0.0 xoffset 30 yalign 1.0 yoffset 50 xzoom 0.5 yzoom 0.5
...
screen name_input_screen():
    ...
    frame:
        ...
        vbox:
            ...
            text "Qual seu nome?" size 60
            text "(Use seu teclado para preencher, aperte ENTER ou clique em Confirmar)" size 40

            input id "name_field" value VariableInputValue("player_input") length 20 size 60 color "#fcc200" outlines [(2, "#000000", 0, 0)] xalign 0.5

            textbutton "Confirmar" action Return(player_input) xalign 0.5

    key "K_RETURN" action Return(player_input)


init python:
    style.name_input_frame.background = Frame("gui/frame.png", 20, 20)
    style.name_input_frame.xmaximum = 1000

    style.name_input_text.color = "#FFFFFF"
    style.name_input_text.size = 80

    style.name_input_input.size = 94
    style.name_input_input.color = "#FFFFFF"
    style.name_input_input.outlines = [(2, "#000000", 0, 0)]
    style.name_input_input.xminimum = 600
    style.name_input_input.xmaximum = 800
    style.name_input_input.xfill = True 


    style.name_input_button.xminimum = 400   # Largura mínima
    style.name_input_button.yminimum = 120   # Altura mínima
    style.name_input_button.padding = (40, 20)
    style.name_input_button.background = "#880088"
    style.name_input_button.hover_background = "#AA00AA"
    style.name_input_button_text.color = "#FFFFFF"
    style.name_input_button_text.size = 60
